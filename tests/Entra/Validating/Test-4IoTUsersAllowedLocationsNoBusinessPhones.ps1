function Test-4IoTUsersAllowedLocationsNoBusinessPhones {
    param(
    ...
    try {
        ...
        foreach ($Group in $Groups) {
            ...
            $users += Get-MtGroupMember -GroupId $group.id | %{Invoke-MtGraphRequest -RelativeUri "users" -Filter "id eq '$($_.id)'" -Select $fieldsToSelect}
        }
        $invalidUsers = @()
