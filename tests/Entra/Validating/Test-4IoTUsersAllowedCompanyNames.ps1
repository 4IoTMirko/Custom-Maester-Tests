function Test-4IoTUsersAllowedCompanyNames {
    param(
    ...
    try {
        ...
        foreach ($Group in $Groups) {
            $users += Get-MtGroupMember -GroupId $group.id | ForEach-Object{Invoke-MtGraphRequest -RelativeUri "users" -Filter "id eq '$($_.id)'" -Select id,displayName,jobTitle,companyName,postalCode,streetaddress,state,city,country,businessPhones,department,officeLocation,mobilePhone,employeeHireDate,employeeID,sponsors,mail,othermails,proxyaddresses}
        }
        $invalidUsers = @()
